...
export const WithHeader = ({ children }:any) => {
  const [hasBackground, setHasBackground] = useState(false);
  const user = useAppSelector(redux.storeParts.user.getData);

  const handleScroll = () => {
    const scrollPosition = window.scrollY; // => scroll position
    if (scrollPosition < 20) return setHasBackground(false);
    if (scrollPosition > 20) return setHasBackground(true);
  };
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:title" content="CoWorking" />
      </Helmet>
      <nav data-role="Header" className={`${hasBackground ? 'withBackground' : ''} landing-page-navbar`}>
        <div>
          <h1>CoWorker</h1>
          <div className="landing-page-right-side">
            ...
            <Link to={route('/profile')} className="landing-page-profile">
              ...
              /> : <AccountCircleIcon style={{ width:40, height:40 }} color="action"/>}
              <div className="landing-page-container1">
                <span className="landing-page-text05">{user?.name}</span>
                <span className="landing-page-text06">View Profile</span>
              </div>
            </Link>
          </div>
            ...
            <div className="landing-page-nav">
              ...
              <div className="landing-page-right-side1">
                <div className="landing-page-links-container1">
                  <span className="landing-page-text07">Home</span>
                  <span className="landing-page-text08">About</span>
                  <span className="landing-page-text09">Tour Packages</span>
                  <span>Contact</span>
                </div>
                <a href="#main-section" className="landing-page-link01">
                  <SolidButton>Explore places</SolidButton>
                </a>
              </div>
            </div>
